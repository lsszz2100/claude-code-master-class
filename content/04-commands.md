...
| `/agents` | 서브에이전트 관리 |
| `/output-style` | 출력 스타일(기본, 설명형, 학습형 등) 목록 열람 및 전환 |
| `/reload-skills` | 세션 재시작 없이 새로 추가·수정된 스킬을 즉시 다시 로드 |
| `/remote-control` | 웹(claude.ai)이나 모바일 앱에서 로컬 세션을 원격 제어하도록 연결 |
...
| `/status` | 현재 세션 상태 확인 |
| `/usage` | API 사용량·비용 표시 (`/usage-credits`로 잔여 크레딧 확인) |
| `/focus` | 프롬프트와 1줄 작업 요약, 최종 응답만 남기는 초경량 포커스 뷰 토글 |
| `/code-review` | 변경 diff를 새 서브에이전트가 검토(번들 스킬, `--fix`로 수정 적용) |
| `/security-review` | diff의 보안 취약점 점검(번들 스킬) |
| `/code-review ultra` | 같은 리뷰를 **클라우드에서 다중 에이전트로** 깊게(지적마다 재현·검증). 사용량 크레딧 과금 — [17장](#ch17) |
