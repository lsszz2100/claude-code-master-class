...
QUIZZES = {
    ...
    16: [
        ...
        ("`Bash(ls *)`와 `Bash(ls*)`의 차이는?",
         ["같은 규칙이다", "앞은 `lsof`에 안 맞고 뒤는 맞는다", "앞은 하위 디렉터리까지 본다", "뒤는 문법 오류다"], 1,
         "끝에 붙은 ` *`(공백+별표)는 **단어 경계**를 요구합니다. `Bash(ls *)`는 `ls -la`에만 맞고 `lsof`에는 안 맞지만, 공백 없는 `Bash(ls*)`는 둘 다 맞습니다."),
        ("프롬프트 캐시가 적중(hit)했을 때 입력 토큰 요금은 기본가의 몇 배인가요?",
         ["0.1배", "0.5배", "1배(동일)", "1.25배"], 0,
         "캐시 읽기는 기본 입력가의 **0.1배**입니다(5분 캐시 쓰기 1.25배, 1시간 2배). 그래서 5분 캐시는 한 번만 재사용해도 이득입니다. 계산기 위젯의 캐시 슬라이더가 이 배수로 계산합니다."),
        ("컨텍스트 예산에서 서브에이전트로 작업을 격리하면 무엇이 줄어드나요?",
         ["시스템 프롬프트", "CLAUDE.md", "도구 출력이 메인 컨텍스트에 쌓이는 양", "모델의 컨텍스트 창 크기"], 2,
         "서브에이전트는 **자기 컨텍스트에서 일하고 요약만 돌려줍니다.** 그래서 탐색처럼 출력이 큰 작업을 격리하면 메인 컨텍스트에 쌓이는 도구 출력이 크게 줄어듭니다."),
    ],
}
...
/* 미션 바 — 터미널 내부는 테마와 무관하게 항상 어둡다. 그래서 변수가 아닌 고정색을 쓴다 */
.pg-mission{display:flex;align-items:center;gap:9px;flex-wrap:wrap;padding:10px 14px;
  background:#12131a;border-bottom:1px solid #24262f;font-size:13px;color:#cfcabb}
.pg-mission .n{font-size:11.5px;color:#d9b775;border:1px solid #3a3d49;border-radius:20px;padding:3px 9px;white-space:nowrap}
.pg-mission .t{flex:1;min-width:170px;line-height:1.5}
.pg-mission .t b{color:#f2efe8;font-weight:600}
.pg-mission button{font:inherit;font-size:11.5px;cursor:pointer;background:transparent;
  border:1px solid #3a3d49;color:#a7a396;border-radius:7px;padding:4px 10px}
.pg-mission button:hover{border-color:#e07a4b;color:#e07a4b}
.pg-mission.clear .n{color:#4cc189;border-color:#4cc189}
...
.pg-out .dim{color:#78756c;font-size:12.5px}
...
.pg-tip{margin-top:9px;font-size:12.5px;color:var(--ink-dim);line-height:1.9}
.pg-tip kbd{font-family:"SF Mono",monospace;font-size:11.5px;background:var(--code-bg);color:var(--ink);
...
.pg-step{font-family:"SF Mono",monospace;font-size:11.5px;letter-spacing:1px;color:var(--accent2);margin-bottom:9px}
...
.pg-result pre{margin:0 0 13px}
.pg-also{margin:0 0 13px;padding:11px 14px;font-size:13.5px;line-height:1.65;color:var(--ink-dim);
  background:var(--code-bg);border:1px solid var(--line);border-left:3px solid var(--accent2);border-radius:9px}
.pg-also b{color:var(--ink)}
.pg-nav{display:flex;flex-wrap:wrap;gap:8px;margin-top:13px}
.pg-nav button{font:inherit;font-size:13px;cursor:pointer;background:transparent;border:1px solid var(--line);color:var(--ink-dim);border-radius:9px;padding:8px 14px}
.pg-nav button:hover{border-color:var(--accent);color:var(--accent)}
...
.pg-cost .toggles{display:flex;flex-wrap:wrap;gap:9px 20px;margin:0 0 17px}
.pg-cost .toggles label{display:flex;align-items:center;gap:8px;font-size:13.5px;color:var(--ink);cursor:pointer}
.pg-cost .toggles input{width:16px;height:16px;accent-color:var(--accent);cursor:pointer;flex:none}
.pg-cost .toggles .val{color:var(--accent2);font-weight:700;font-family:"SF Mono",monospace;font-size:12.5px}
.pg-cost .save{color:#3aa76d;font-weight:700}
.pg-cmp{margin-top:17px}
.pg-cmp .h{font-size:13px;color:var(--ink-dim);margin-bottom:11px}
.pg-cmp .row{display:grid;grid-template-columns:92px 1fr 94px;align-items:center;gap:11px;margin-bottom:8px;font-size:13px}
.pg-cmp .nm{color:var(--ink-dim);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.pg-cmp .bar{height:9px;background:var(--panel2);border:1px solid var(--line);border-radius:20px;overflow:hidden}
.pg-cmp .bar i{display:block;height:100%;background:var(--accent2);border-radius:20px}
.pg-cmp .amt{text-align:right;font-family:"SF Mono",monospace;font-size:12.5px;color:var(--ink-dim);white-space:nowrap}
...
if(pgt){
  pgt.innerHTML='<div class="pg-bar"><i></i><i></i><i></i><span>claude — 놀이터</span></div><div class="pg-mission" id="pgMis"></div><div class="pg-out" id="pgOut"></div><div class="pg-inline"><span class="p">❯</span><input id="pgIn" type="text" aria-label="놀이터 터미널 명령 입력" placeholder="/help 를 입력해 보세요" autocomplete="off" spellcheck="false"></div>';
  const out=pgt.querySelector('#pgOut'), inp=pgt.querySelector('#pgIn'), mis=pgt.querySelector('#pgMis');
  const MODELS=['opus','sonnet','haiku','fable'], EFFORTS=['low','medium','high','xhigh','max'];
  // 값은 전부 시뮬레이션이지만 명령의 "역할"은 실제와 맞춘다 — 여기서 잘못 외우면 그대로 굳는다
  const CMDS={
    '/help':()=>['<span class="k">사용 가능한 명령</span>',
      '<span class="k">컨텍스트</span>  /clear · /compact · /context · /rewind · /resume · /export',
      '<span class="k">모델·성능</span>  /model · /effort · /fast · /cost · /usage',
      '<span class="k">설정·확장</span>  /init · /memory · /agents · /mcp · /hooks · /permissions · /config · /add-dir',
      '<span class="k">개발 도구</span>  /review · /pr-comments · /todos · /ide · /vim · /output-style · /statusline',
      '<span class="k">진단·계정</span>  /status · /doctor · /login · /logout · /bug · /terminal-setup · /exit',
      '<span class="dim">아무 문장이나 입력하면 Claude가 작업을 시작합니다(시뮬레이션).</span>'],
    '/clear':()=>{out.innerHTML='';return['<span class="ok">✓ 컨텍스트를 리셋했습니다.</span> 새 대화를 시작합니다.'];},
    '/context':()=>['<span class="k">컨텍스트</span> 42,300 / 1,000,000 토큰 (4%) 사용',
      '<span class="dim">시스템 12.4K · CLAUDE.md 3.1K · 도구 정의 8.8K · 대화 18.0K</span>',
      '여유롭습니다. 길어지면 /compact 로 압축하세요.'],
    '/compact':a=>['<span class="ok">✓ 대화를 요약해 컨텍스트를 확보했습니다.</span> 핵심 결정·미해결 이슈는 보존됩니다.'].concat(
      a?['<span class="dim">요약 지침 반영: “'+esc(a)+'”</span>']:['<span class="dim">/compact 뒤에 문장을 붙이면 “무엇을 남길지” 지정할 수 있습니다.</span>']),
    '/rewind':()=>['<span class="k">되감기</span> 체크포인트 3개 — 코드만 · 대화만 · 둘 다 중에서 고릅니다.','<span class="dim">Esc 를 두 번 눌러도 열립니다.</span>'],
    '/resume':()=>['<span class="k">이전 대화</span> 3건 — 목록에서 골라 그 컨텍스트로 이어서 시작합니다.','<span class="dim">터미널을 껐다 켜도 대화는 남아 있습니다.</span>'],
    '/export':()=>['<span class="ok">✓ 대화를 내보냈습니다.</span> 파일 또는 클립보드로 저장합니다.'],
    '/model':a=>a?['<span class="ok">✓ 모델을 '+esc(a)+' 로 설정</span>했습니다.']
      :['<span class="k">모델</span> opus · sonnet · haiku · fable 중 선택. 예: /model opus'],
    '/effort':a=>a?(EFFORTS.includes(a.toLowerCase())
        ?['<span class="ok">✓ effort '+esc(a)+' 적용.</span> 어려운 코딩·에이전틱 작업엔 xhigh 권장.']
        :['<span class="err">알 수 없는 effort:</span> low·medium·high·xhigh·max 중에서 고르세요.'])
      :['<span class="k">effort</span> 기본 high. 예: /effort xhigh','<span class="dim">응답 길이 레버가 아니라 “얼마나 깊이 생각할지”입니다. 바꾸면 프롬프트 캐시가 무효화됩니다.</span>'],
    '/fast':()=>['<span class="ok">✓ 빠른 모드 ON</span> — Opus를 더 빠르게(프리미엄). 작은 모델로 내려가는 게 아닙니다.'],
    '/cost':()=>['<span class="k">이 세션</span> 입력 128,400 · 출력 34,200 토큰 · 약 $1.49','<span class="dim">캐시 읽기 91,000 토큰이 포함돼 있습니다(입력가의 1/10).</span>'],
    '/usage':()=>['<span class="k">사용량</span> 이번 주 한도의 38% 사용 · 초기화까지 3일'],
    '/init':()=>['<span class="ok">✓ CLAUDE.md 생성됨.</span> 빌드 명령·테스트·규칙을 감지해 시작 파일을 만들었습니다.','<span class="dim">생성된 내용은 반드시 읽고 다듬으세요 — 추측이 섞여 있습니다.</span>'],
    '/memory':()=>['<span class="k">기억</span> MEMORY.md 색인 로드됨. 자동 기억 ON.','<span class="dim">CLAUDE.md 는 “프로젝트의 규칙”, 기억은 “나에 대해 배운 것”입니다.</span>'],
    ...
    '/pr-comments':()=>['<span class="k">PR 코멘트</span> 열린 PR 의 리뷰 코멘트를 가져와 대화로 들여옵니다.'],
    '/todos':()=>['<span class="k">할 일</span> ① 인증 미들웨어 작성 <span class="dim">(진행 중)</span>','② 테스트 추가 <span class="dim">(대기)</span>','③ 문서 갱신 <span class="dim">(대기)</span>'],
    '/ide':()=>['<span class="k">IDE</span> 연결된 편집기 없음. VS Code·JetBrains 확장을 설치하면 선택 영역·진단이 함께 전달됩니다.'],
    ...
    '/bug':()=>['<span class="k">버그 신고</span> 대화 기록을 첨부해 이슈를 엽니다.'],
    '/terminal-setup':()=>['<span class="ok">✓ Shift+Enter 줄바꿈 키 바인딩을 설치했습니다.</span>'],
    '/exit':()=>['<span class="sys">세션을 종료합니다. (놀이터라 실제로 닫히지는 않습니다)</span>'],
  };
  ...
  const MIS=[
    {t:'컨텍스트가 얼마나 찼는지 확인해 보세요.',ok:c=>c.base==='/context'},
    {t:'대화가 길어졌습니다. 지금까지를 <b>요약해 압축</b>하세요.',ok:c=>c.base==='/compact'},
    {t:'까다로운 리팩터링을 앞뒀습니다. 추론 강도를 <b>xhigh</b> 로 올리세요.',ok:c=>c.base==='/effort'&&c.arg.toLowerCase()==='xhigh'},
    {t:'새 저장소에 붙였습니다. <b>프로젝트 지침 파일</b>을 만드세요.',ok:c=>c.base==='/init'},
    {t:'방금 수정이 잘못됐습니다. <b>체크포인트로 되돌리는</b> 화면을 여세요.',ok:c=>c.base==='/rewind'},
    {t:'이 세션에서 <b>돈이 얼마나 나갔는지</b> 확인하세요.',ok:c=>c.base==='/cost'},
  ];
  let mi=0, misOn=true;
  function misBtn(id,label){ return '<button type="button" id="'+id+'">'+label+'</button>'; }
  function renderMis(){
    mis.className='pg-mission';
    if(!misOn){
      mis.innerHTML='<span class="t">미션 모드가 꺼져 있습니다. 자유롭게 명령을 쳐 보세요.</span>'+misBtn('pgMisOn','미션 켜기');
      mis.querySelector('#pgMisOn').addEventListener('click',()=>{misOn=true;mi=0;renderMis();inp.focus();});
      return;
    }
    if(mi>=MIS.length){
      mis.className='pg-mission clear';
      mis.innerHTML='<span class="n">완료 '+MIS.length+'/'+MIS.length+'</span><span class="t"><b>미션을 모두 끝냈습니다.</b> 이제 /help 로 나머지도 둘러보세요.</span>'+misBtn('pgMisAgain','다시 하기');
      mis.querySelector('#pgMisAgain').addEventListener('click',()=>{mi=0;renderMis();inp.focus();});
      return;
    }
    mis.innerHTML='<span class="n">미션 '+(mi+1)+'/'+MIS.length+'</span><span class="t">'+MIS[mi].t+'</span>'
      +misBtn('pgMisSkip','건너뛰기')+misBtn('pgMisOff','끄기');
    mis.querySelector('#pgMisSkip').addEventListener('click',()=>{mi++;renderMis();inp.focus();});
    mis.querySelector('#pgMisOff').addEventListener('click',()=>{misOn=false;renderMis();inp.focus();});
  }

  function run(cmd){
    cmd=cmd.trim(); if(!cmd) return;
    print(['❯ '+esc(cmd)],'u');
    const parts=cmd.split(/\s+/), base=parts[0].toLowerCase(), arg=parts.slice(1).join(' ');
    if(base[0]==='/'){
      const fn=CMDS[base];
      if(fn) print(fn(arg),'sys');
      else {
        const near=ALL.filter(c=>c.startsWith(base));
        print(['<span class="err">명령을 찾을 수 없습니다:</span> '+esc(base)
          +(near.length?' — 혹시 <span class="k">'+near.slice(0,4).join('</span> · <span class="k">')+'</span>?'
                       :' — <span class="k">/help</span> 를 입력해 보세요.')]);
      }
    } else {
      print(['<span class="sys">Claude가 작업을 시작합니다… (시뮬레이션)</span>',
        '<span class="dim">1) 탐색 — 관련 파일을 찾습니다 (Glob·Grep·Read)</span>',
        '<span class="dim">2) 계획 — 무엇을 어떻게 고칠지 정리합니다</span>',
        '<span class="dim">3) 구현 — Edit·Write 로 코드를 고칩니다</span>',
        '<span class="dim">4) 검증 — 테스트·빌드를 돌려 스스로 확인합니다</span>',
        '<span class="sys">4)가 빠지면 “됐다고 하는데 안 되는” 결과가 나옵니다. 검증 수단을 꼭 쥐여 주세요.</span>']);
    }
    if(misOn&&mi<MIS.length&&MIS[mi].ok({base,arg,cmd})){
      print(['<span class="ok">✓ 미션 '+(mi+1)+' 완료!</span>'],'sys'); mi++; renderMis();
    }
  }

  // --- 히스토리 · 자동완성 ---
  const hist=[]; let hp=0;
  const caretEnd=()=>inp.setSelectionRange(inp.value.length,inp.value.length);
  function lcp(a){ let s=a[0]; a.forEach(x=>{ while(s&&!x.startsWith(s)) s=s.slice(0,-1); }); return s; }
  function complete(){
    const v=inp.value, parts=v.split(/\s+/);
    if(parts.length>1){                       // 인자 자리
      const pool=ARGV[parts[0].toLowerCase()]; if(!pool) return;
      const hit=pool.filter(x=>x.startsWith(parts[1].toLowerCase()));
      if(hit.length===1){ inp.value=parts[0]+' '+hit[0]; caretEnd(); }
      else if(hit.length>1) print(['<span class="dim">'+hit.join('   ')+'</span>']);
      return;
    }
    const hit=ALL.filter(c=>c.startsWith(v.toLowerCase()));
    if(!hit.length) return;
    if(hit.length===1){ inp.value=hit[0]+(ARGV[hit[0]]?' ':''); caretEnd(); return; }
    inp.value=lcp(hit); caretEnd();
    print(['<span class="dim">'+hit.join('   ')+'</span>']);
  }
  inp.addEventListener('keydown',e=>{
    if(e.key==='Enter'){
      const v=inp.value;
      if(v.trim()){ hist.push(v); if(hist.length>50) hist.shift(); }
      hp=hist.length; run(v); inp.value='';
    }
    // Tab 은 슬래시 명령을 치는 중일 때만 가로챈다 — 그래야 키보드로 입력창을 빠져나갈 수 있다
    else if(e.key==='Tab'&&!e.shiftKey&&inp.value.trim()[0]==='/'){ e.preventDefault(); complete(); }
    else if(e.key==='ArrowUp'&&hist.length){ e.preventDefault(); hp=Math.max(0,hp-1); inp.value=hist[hp]; caretEnd(); }
    else if(e.key==='ArrowDown'&&hist.length){ e.preventDefault(); hp=Math.min(hist.length,hp+1); inp.value=hp>=hist.length?'':hist[hp]; caretEnd(); }
    else if((e.key==='l'||e.key==='L')&&(e.ctrlKey||e.metaKey)){ e.preventDefault(); out.innerHTML=''; }
  });
  renderMis();
  ...
  ['/help','/context','/compact','/effort xhigh','/cost','/review','/clear'].forEach(c=>{const b=document.createElement('button');b.type='button';b.textContent=c;b.addEventListener('click',()=>{run(c);inp.focus();});chips.appendChild(b);});
  const tip=document.createElement('div'); tip.className='pg-tip';
  tip.innerHTML='<kbd>Tab</kbd> 자동완성 · <kbd>↑</kbd> <kbd>↓</kbd> 명령 히스토리 · <kbd>Ctrl</kbd>+<kbd>L</kbd> 화면 지우기 — 실제 Claude Code에서도 그대로 통합니다.';
  pgt.after(tip); pgt.after(chips);
}

// ===== 플레이그라운드 · 확장 진단 위저드 =====
const pgw=document.querySelector('.pg-wizard');
if(pgw){
  // 결과마다 "그래서 뭘 만들면 되는지"까지 준다 — 추천만 하고 끝나면 실제로 안 해 본다
  const RES={
    claudemd:{t:'CLAUDE.md',d:'매번 다시 설명하게 되는 <strong>사실·규칙</strong>이라면 프로젝트 지침 파일에 담으세요. 짧고 구체적으로(200줄 이하) 유지하는 게 핵심입니다.',href:'#ch5',
      f:'CLAUDE.md',code:'# 프로젝트\n\n## 빌드·테스트\n- 빌드: `pnpm build`\n- 테스트: `pnpm test -- --run` (watch 모드 금지)\n\n## 규칙\n- 새 의존성 추가 전에 먼저 물어볼 것\n- API 응답 타입은 src/types/api.ts 한 곳에서만 정의',
      also:'<b>함께:</b> 지침이 “지켜지면 좋겠다”가 아니라 <b>반드시</b> 지켜져야 한다면 그건 CLAUDE.md가 아니라 훅의 일입니다.'},
    skill:{t:'스킬(Skill)',d:'가끔 필요한 <strong>다단계 절차·도메인 지식</strong>이라면 온디맨드로 로드되는 스킬로. 이름이 곧 슬래시 명령이 됩니다.',href:'#ch8',
      f:'.claude/skills/deploy/SKILL.md',code:'---\nname: deploy\ndescription: 스테이징에 배포한다. "배포", "deploy", "스테이징에 올려" 라고 하면 발동.\n---\n\n1. `pnpm test -- --run` 이 통과하는지 먼저 확인한다.\n2. `pnpm build` 로 빌드한다.\n3. `vercel deploy --prebuilt` 로 올린다.\n4. 배포 URL 을 사용자에게 보고한다.',
    ...
    'sub-builtin':{t:'내장 서브에이전트 (Explore · Plan)',d:'코드 위치 찾기·계획 세우기라면 <strong>이미 있는 것</strong>으로 충분합니다. 탐색 과정의 긴 출력은 격리되고 결론만 메인 대화로 돌아옵니다.',href:'#ch7',
      f:'대화에서 바로',code:'# 위치만 알고 싶을 때\nExplore 에이전트로 결제 재시도 로직이 어디 있는지 찾아줘\n\n# 손대기 전에 설계부터\nPlan 에이전트로 이 마이그레이션 순서를 짜줘',
      also:'<b>주의:</b> 서브에이전트는 결론만 돌려줍니다. 중간 근거가 필요하면 “찾은 파일 경로도 같이 보고해”라고 명시하세요.'},
  ...
  const QS={
    start:{q:'지금 상황에 가장 가까운 것은?',opts:[
      ['같은 지시를 매번 반복해서 입력한다','q:repeat'],
    ...
    repeat:{q:'그 반복되는 내용은 어느 쪽에 가깝나요?',opts:[
      ['항상 지켜야 할 사실·규칙(빌드 명령·코드 스타일 등)','r:claudemd'],
  ...
  function renderQ(key){
    ...
    Q.opts.forEach(([label,next])=>{
      ...
      b.addEventListener('click',()=>{
        if(next.slice(0,2)==='q:'){ path.push(next.slice(2)); renderQ(next.slice(2)); }
        else showRes(next.slice(2));
      });
      box.appendChild(b);
    });
    if(path.length>1) addNav([['뒤로',()=>{ path.pop(); renderQ(path[path.length-1]); }]]);
  }
  function addNav(items){
    const nav=pgw.querySelector('.pg-nav');
    items.forEach(([label,fn])=>{const b=document.createElement('button');b.type='button';b.textContent=label;b.addEventListener('click',fn);nav.appendChild(b);});
  }
  function showRes(key){
    const r=RES[key];
    pgw.innerHTML='<div class="pg-result"><span class="tag">추천</span><h4>'+r.t+'</h4><p>'+r.d+'</p>'
      +'<div class="pg-step">'+esc2(r.f)+'</div>'
      +'<div class="pg-code"></div>'
      +'<div class="pg-also">'+r.also+'</div>'
      +'<a href="'+r.href+'">해당 챕터로 →</a><div class="pg-nav"></div></div>';
    // 코드 블록은 문자열이 아니라 DOM 으로 만든다 — 소스에 </pre> 리터럴이 들어가면
    // prerender.mjs 의 mermaid 수집 정규식이 위쪽 주석의 <pre class="mermaid"> 와 짝지어
    // 스크립트 전체를 도표로 착각한다.
    const pre=document.createElement('pre'), code=document.createElement('code');
    code.textContent=r.code; pre.appendChild(code);
    pgw.querySelector('.pg-code').replaceWith(pre);
    addNav([
      ['뒤로',()=>renderQ(path[path.length-1])],
      ['처음부터 다시',()=>{ path=['start']; renderQ('start'); }],
    ]);
    const link=pgw.querySelector('.pg-result a'); link.addEventListener('click',e=>{e.preventDefault(); navTo(link.getAttribute('href'));});
    // 복사 버튼 일괄 부착은 이 위젯이 그려지기 전에 이미 끝났으므로 여기서 직접 단다
    const btn=document.createElement('button');
    btn.className='copy-btn'; btn.type='button'; btn.textContent='복사';
    btn.addEventListener('click',async()=>{
      try{await navigator.clipboard.writeText(r.code);}catch(_){}
      btn.textContent='✓ 복사됨'; btn.classList.add('done');
      setTimeout(()=>{btn.textContent='복사';btn.classList.remove('done');},1400);
    });
    pre.appendChild(btn);
...
if(pgc){
  ...
  pgc.innerHTML=
    ...
    '<div class="fld"><div class="lab"><b>③ 내가 보내는 양 (입력)</b><span class="val" id="ccInV"></span></div><input id="ccIn" aria-label="입력 토큰 수" type="range" min="200" max="80000" step="200" value="4000"></div>'+
  ...
  // 요청 1건 값. 모델별 캐시 요율(m[4]) 적용, 없으면 기본 CACHE_READ(0.1)
  function perReq(m,i,o,h,b){
    const cr = (m[4] !== undefined) ? m[4] : CACHE_READ;
    const p=(i*(1-h)/1e6*m[0])+(i*h/1e6*m[0]*cr)+(o/1e6*m[1]);
    return b?p*BATCH:p;
  }
  function calc(){
    const m=M[sel.value], n=+req.value, i=+ti.value, o=+to.value, h=+hit.value/100, b=batch.checked;
    ...
    if(day<base-1e-9) brk+='<br><span class="save">↓ 캐싱·배치로 '+Math.round((1-day/base)*100)+'% 절감 (원가 '+won(base)+'/일)</span>';
    pgc.querySelector('#ccBrk').innerHTML=brk;
    pgc.querySelector('#ccNote').innerHTML='캐시에 <b>처음 쓸 때</b>는 입력가의 1.25배가 붙습니다 — 같은 접두사를 여러 번 재사용할 때만 이득입니다.'
